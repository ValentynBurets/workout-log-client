...
export function ScheduledTrainingPlanReducer(state: ScheduledTrainingPlan, action: any): ScheduledTrainingPlan {
  switch (action.type) {
    case 'SET_NAME':
    ...
    case 'ADD_DAY_SCHEDULE':
      return { ...state, daySchedules: [...state.daySchedules, action.payload] };
    case 'REMOVE_DAY_SCHEDULE':
      return { ...state, daySchedules: state.daySchedules.filter((_, index) => index !== action.payload) };
